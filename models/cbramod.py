...
https://github.com/wjq-learning/CBraMod adapted by Vita Shaw
"""
...
class CBraModEncoder(nn.Module):
    def __init__(self, in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
                 nhead=8, drop_rate=0.1, norm_group=5):
        super().__init__()
        self.patch_embedding = PatchEmbedding(
            in_dim, out_dim, d_model, seq_len, norm_group, dropout=drop_rate)
        encoder_layer = TransformerEncoderLayer(
            d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, batch_first=True, norm_first=True,
            activation=F.gelu, dropout=drop_rate
        )
...
class PatchEmbedding(nn.Module):
    def __init__(self, in_dim, out_dim, d_model, seq_len, norm_group=5, dropout=0.1):
        super().__init__()
        self.d_model = d_model
        self.positional_encoding = nn.Sequential(
            nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=(seq_len, 7),
                      stride=(1, 1), padding="same", groups=d_model),
        )
        ...
        self.proj_in = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=out_dim, kernel_size=(
                1, 49), stride=(1, 25), padding=(0, 24)),
            nn.GroupNorm(norm_group, out_dim),
            nn.GELU(),

            nn.Conv2d(in_channels=out_dim, out_channels=out_dim, kernel_size=(
...
class CBraMod(nn.Module):
    def __init__(self, in_channels=19, n_classes=2, n_samples=2000,
                 out_channels_init=25, emb_size=200, drop_rate=0.1, 
                 use_pretrained=None, **kwargs):
        super().__init__()
        self.input = Rearrange('b 1 c (p s) -> b c p s', s=200)
        
        self.backbone = CBraModEncoder(
            seq_len=in_channels, out_dim=out_channels_init, 
            drop_rate=drop_rate, **kwargs)
        if use_pretrained is not None:
            self.backbone.load_state_dict(torch.load(
                use_pretrained, map_location='cuda'))
            # self.backbone.requires_grad_(False)
        self.backbone.proj_out = nn.Identity()
        
        n_patch = n_samples // 200
        
        self.classifier = nn.Sequential(
            Rearrange('b c s d -> b (c s d)'),
            nn.Linear(in_channels * n_patch * emb_size, n_patch * emb_size),
            nn.ELU(),
            nn.Dropout(drop_rate),
            nn.Linear(n_patch * emb_size, emb_size),
            nn.ELU(),
            nn.Dropout(drop_rate),
