"""
Ding, Yi, et al. "EEG-Deformer: A dense convolutional transformer for brain-computer interfaces." 
    IEEE Journal of Biomedical and Health Informatics (2024).

https://github.com/yi-ding-cs/EEG-Deformer adapted by Vita Shaw
"""
...
class PatchEmbedding(nn.Module):
    def __init__(self, out_chan, kernel_size, num_chan, num_time, downsample=2):
        super().__init__()

        self.cnn_encoder = nn.Sequential(
            Conv2dWithConstraint(1, out_chan, kernel_size,
                                 padding=(0, int(0.5 * (kernel_size[-1] - 1))),
                                 max_norm=2,
                                 ),
            ...
            nn.ELU(),
            nn.MaxPool2d((1, downsample), stride=(1, downsample)),
        )

        self.to_patch_embedding = Rearrange('b k c f -> b k (c f)')

        self.pos_embedding = nn.Parameter(
            torch.randn(1, out_chan, num_time // downsample))
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, in_chan, fine_grained_kernel=11, dropout=0.):
        ...
        for i in range(depth):
            ...
            self.layers.append(nn.ModuleList([
                ...
                self.cnn_block(in_chan=in_chan,
                               kernel_size=fine_grained_kernel, dp=dropout)
            ]))
...
class EEGDeformer(nn.Module):
    def __init__(self, in_channels=32, n_classes=2, n_samples=1000, 
                 temporal_kernel=11, num_kernel=64, downsample=2, 
                 depth=4, heads=16, mlp_dim=16, dim_head=16, drop_rate=0.5):
        super().__init__()
        ...
        dim = n_samples // downsample
        
        self.transformer = Transformer(
            dim=dim, depth=depth, heads=heads, dim_head=dim_head,
            mlp_dim=mlp_dim, dropout=drop_rate,
            in_chan=num_kernel, fine_grained_kernel=temporal_kernel,
        )

        L = [int(dim * (0.5 ** i)) for i in range(depth + 1)]
